...
int sys826_readver(uint index, uint *version)
{
    ...
    if (DeviceIoControl(ixinfo[index].hboard, S826_IOC_VERSION, &ver, sizeof (struct s826_version), &ver, sizeof (struct s826_version), &nBytes, NULL) != 1)
...
static int GetBoardAddress(HANDLE hboard, PVOID *addr)
{
    ...
    // Get board's base memory address.
    param.idx = 1;
    if (DeviceIoControl(hboard, S826_IOC_GETMEM, &param, sizeof (struct s826_getmem), &param, sizeof (struct s826_getmem), &nBytes, NULL) != 1)
...
int sys826_open()
{
    ...
    for (b = 0; b < S826_MAX_BOARDS; b++)    // For each driver-enumerated board (in range 0 to boardcount-1) ...
    {
        ...
        // Get board's base memory address.
        errnum = GetBoardAddress(ixinfo[b].hboard, &ixinfo[b].baseadr);
...
int sys826_wait_counter(uint index, uint intlist, uint timeout)
{
	...
	p.bWaitAny  = 0;            // wait all
    return DeviceIoControl(ixinfo[index].hboard, S826_IOC_WAITCOUNT, &p, sizeof (struct s826_waitcount), &p, sizeof (struct s826_waitcount), &nBytes, NULL) ? p.rc : S826_ERR_DRIVER;
}
...
int sys826_wait_dio(uint index, uint *intlist, uint waitany, uint timeout)
{
    ...
    return DeviceIoControl(ixinfo[index].hboard, S826_IOC_WAITDIO, &p, sizeof (struct s826_waitdio), &p, sizeof (struct s826_waitdio), &nBytes, NULL) ? p.rc : S826_ERR_DRIVER;
}
...
int sys826_wait_adc(uint index, uint intlist, uint waitany, uint timeout)
{
    ...
    p.mask      = intlist;      // interrupt flag(s) to wait for
	p.bWaitAny  = (waitany != 0) ? 1 : 0;
    return DeviceIoControl(ixinfo[index].hboard, S826_IOC_WAITADC, &p, sizeof (struct s826_waitadc), &p, sizeof (struct s826_waitadc), &nBytes, NULL) ? p.rc : S826_ERR_DRIVER;
}
...
int sys826_wait_watchdog(uint index, uint timeout)
{
    ...
    p.usecs = timeout;      // microseconds
    return DeviceIoControl(ixinfo[index].hboard, S826_IOC_WAITWD, &p, sizeof (struct s826_waitwd), &p, sizeof (struct s826_waitwd), &nBytes, NULL) ? p.rc : S826_ERR_DRIVER;
}
...
int sys826_waitcancel_adc(uint index, uint slotlist)
{
    ...
    BOOL success;
    p.mask = slotlist;
    success = DeviceIoControl(ixinfo[index].hboard, S826_IOC_CANCEL_WAITADC, &p, sizeof(p), &p, sizeof(p), &nBytes, NULL);
...
int sys826_waitcancel_counter(uint index, uint chan)
{
    ...
    BOOL success;
    p.mask = (1 << chan);
    success = DeviceIoControl(ixinfo[index].hboard, S826_IOC_CANCEL_WAITCOUNT, &p, sizeof(p), &p, sizeof(p), &nBytes, NULL);
    return success? S826_ERR_OK : S826_ERR_DRIVER;
}
...
int  sys826_waitcancel_dio(uint index, const uint chanlist[2])
{
    ...
    p.mask[0] = chanlist[0];
    p.mask[1] = chanlist[1];
    success = DeviceIoControl(ixinfo[index].hboard, S826_IOC_CANCEL_WAITDIO, &p, sizeof(p), &p, sizeof(p), &nBytes, NULL);
    return success? S826_ERR_OK : S826_ERR_DRIVER;
}
...
int sys826_waitcancel_watchdog(uint index)
{
    ...
    BOOL success;
    memset(&p, 0, sizeof(p));
    success = DeviceIoControl(ixinfo[index].hboard, S826_IOC_CANCEL_WAITWD, &p, sizeof(p), &p, sizeof(p), &nBytes, NULL);
    return success? S826_ERR_OK : S826_ERR_DRIVER;
}
...
int sys826_writeLCS(uint index, uint offset, uint value, uint rindex, uint rcode)
{
    ...
    success = DeviceIoControl(ixinfo[index].hboard, S826_IOC_SET_LCS, &p, sizeof(p), &p, sizeof(p), &nBytes, NULL);
    return success? S826_ERR_OK : S826_ERR_DRIVER;
}
...
int sys826_readLCS(uint index, uint offset, uint *value, uint rindex, uint rcode)
{
    ...
    p.offset = offset;
    p.reserved[rindex] = rcode;
    success = DeviceIoControl(ixinfo[index].hboard, S826_IOC_GET_LCS, &p, sizeof(p), &p, sizeof(p), &nBytes, NULL);
    *value = p.value;
    return success? S826_ERR_OK : S826_ERR_DRIVER;
}
